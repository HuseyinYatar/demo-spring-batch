...
### Partition thread pool metrics needed manual wiring, and a real Micrometer gotcha

Unlike the job/step timing metrics above, `batchTaskExecutor`'s pool state (active/idle threads, queue depth) is **not** auto-instrumented — confirmed via jar inspection that this Boot version has no `TaskExecutorMetricsAutoConfiguration` for arbitrary `ThreadPoolTaskExecutor` beans. `BatchJobConfig.batchThreadPoolExecutorMetrics` binds it manually via `ExecutorServiceMetrics.monitor(meterRegistry, threadPoolExecutor, "batchTaskExecutor")`, exposing `executor_active_threads`/`executor_pool_size_threads`/`executor_pool_max_threads`/`executor_queued_tasks` (tag `name="batchTaskExecutor"`) — visualized on the "Partition Thread Pool" panels in `batch-metrics-prometheus.json`.

Confirmed live, the hard way: calling `ExecutorServiceMetrics.monitor(...)` inline inside `batchTaskExecutor`'s own `@Bean` method and discarding the result made every one of those gauges read `NaN` — Micrometer's `Gauge` holds its target via a `WeakReference`, and the executor became only weakly reachable despite `batchTaskExecutor` itself being a live Spring singleton (reproduced with a diagnostic print showing the real pool state at registration time, immediately followed by `NaN` on the next scrape). The fix was making the raw `ThreadPoolExecutor` its own `@Bean` (`batchThreadPoolExecutorMetrics`) — Spring's singleton registry then holds an explicit, independent strong reference for the life of the app, and the gauges report real values. If a future executor/gauge integration in this codebase shows `NaN` for no obvious reason, this is the first thing to check.
