...
`TransientInvoiceWriteException` is deliberately *not* registered as skippable, unlike step 1's exceptions. A skip here would be silent — unlike step 1, `buildInvoicesWorkerStep` has no `SkipListener`, so a skipped order would simply never get invoiced, with no audit trail (`rejected-rows.csv` or otherwise) to reveal it happened. For a genuinely transient failure (the kind `retry` exists to recover from), silently dropping the order is worse than failing loudly: once `retryLimit` attempts are exhausted, the exception propagates and fails the chunk/step/job outright, rather than degrading into an invisible per-item skip. This is intentional — the job's failure becomes visible (job execution status, Grafana), and since `buildInvoicesWorkerStep`'s restart correctly resumes only the still-unprocessed orders (verified by `IngestLineItemsRestartTest`/`OrderProcessingJobFailureRestartTest` — restart reads `processed=false` rows only, so nothing already invoiced gets touched again), the safe recovery path is `POST /executions/{id}/restart` once the underlying issue clears, not a silent skip.

### Real DB failures are retried too, and the two steps needed *different* exception types

`TransientInvoiceWriteException`/`FlakyOrderPersistenceSimulator` only demonstrate the retry *mechanism* — they say nothing about whether an actual dropped connection or lock timeout would be retried, because that's a completely different exception type that was never registered. Both worker steps now also retry real DB failures, not skip them (same reasoning as above), but the two steps needed to register **different** exception classes because their writers don't fail the same way:

- **`buildInvoicesWorkerStep`** retries `org.springframework.dao.TransientDataAccessException` and `org.springframework.dao.DataAccessResourceFailureException`. `OrderPersistenceItemWriter` writes via `orderRepository.saveAll(...)`, a genuine Spring Data `JpaRepository` call — Spring's persistence exception translation wraps the underlying Hibernate exception into its own `DataAccessException` hierarchy automatically, so these Spring-level types are what actually gets thrown. Confirmed by `BuildInvoicesDbConnectionRetryTest`.
- **`ingestLineItemsWorkerStep`** retries `org.hibernate.exception.JDBCConnectionException` — Hibernate's own type, *not* a Spring `DataAccessException`. `orderLineItemStagingWriter` is a plain `JpaItemWriter` calling `EntityManager.persist()` directly; unlike a Spring Data repository proxy, `JpaItemWriter` never goes through Spring's exception translation, so the raw Hibernate exception propagates untranslated. **Registering the Spring DAO types here (as was tried first) compiles fine and looks correct, but is silently dead code** — the exception Hibernate actually throws never matches it, so a real connection failure would still fail the job outright instead of retrying. Confirmed by `IngestLineItemsDbConnectionRetryTest`, and only caught because that test was written and run, not assumed.
...
- **`mergeInvoiceSummaryStep`** (a plain `Tasklet`, not a chunk step — a one-shot file merge doesn't need chunk semantics) runs after `buildInvoicesStep` and recombines the per-partition files back into the single `invoice-summary.csv` the job is documented to produce, then deletes the partition files. `InvoiceSummaryMergeTasklet`/`InvoiceSummaryPartitionPaths` (`batch/step2/`) — the latter is shared by both the writer side (`BuildInvoicesStepConfig`) and the merge side so the two can never disagree on the naming scheme, and it discovers partition files by globbing the pattern rather than hardcoding a count, so it's correct regardless of `batch.partition-grid-size`. `perRunStateResetListener` (`BatchJobConfig`) also deletes any stale partition files in `beforeJob`, but only for a genuinely fresh `JobInstance` (`jobExplorer.getJobExecutions(jobExecution.getJobInstance()).size() > 1` detects a restart and skips the cleanup) — necessary because a prior, unrelated run's leftover files (especially from a run with a *different* grid size) would otherwise get scooped up alongside the current run's fresh ones and corrupt the merge. Skipping it on restart matters because `invoiceSummaryCsvItemWriter` resumes appending to its partition file across a restart (its position is restored from the `StepExecution`'s saved `ExecutionContext`) and expects that file to still exist — deleting it out from under a resuming writer throws `ItemStreamException: File is not writable`, which would otherwise break every restart of a `buildInvoicesStep` failure that occurred after at least one chunk had already committed.
